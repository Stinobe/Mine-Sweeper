...
const Optimize: React.FC<OptimizeProps> = ({
  ...
  return (
    <div
      className={classes(
        "bg-white/40 px-4 py-2 rounded-md transition-colors text-gray-700 hover:bg-sky-300 hover:text-white dark:bg-black/40 dark:text-gray-200 dark:hover:bg-black/60 cursor-pointer",
        className || ""
      )}>
      <div
        onClick={onToggle}
        ...
        <div>
          <p className="text-l mb-1">Optimize for screen</p>
          <p className="text-xs font-light opacity-70">
            Generates {maxSize.x} by {maxSize.y} tiles
          </p>
        </div>
        <div>
          <div className="w-7 h-4 bg-black/10 dark:bg-white/10 rounded-full flex border-2 border-transparent">
            <div
              className={classes(
