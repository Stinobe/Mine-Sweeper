...
class Tile {
  ...
  private updateState(): void {
    if (this.isVisible || this.blocked) return;
    switch (this.state) {
    ...
    let flagNum = this.state === TileStates.FLAGGED ? 1 : 0;
    if (this.state === TileStates.MARKED) flagNum = -1;
    GameEvents.trigger(GameEvent.TOGGLE_STATE, flagNum);
    this.informComponent();
  }

  private makeVisible() {
    if (this.blocked && !this.hasMine) return;
